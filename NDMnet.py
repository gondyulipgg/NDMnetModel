...
class ConvBlock(torch.nn.Module):
    def __init__(self, input_size, output_size, kernel_size=4, 
                 stride=2, padding=3, activation=True, batch_norm=True):
...
class DeconvBlock(torch.nn.Module):
    def __init__(self, input_size, output_size, kernel_size=4, stride=2,
                 padding=3, batch_norm=True, dropout=False):
...
class DeconvBlockLast(torch.nn.Module):
    def __init__(self, input_size, output_size, kernel_size=4, 
                 stride=2, padding=3, batch_norm=True, dropout=False):
        super(DeconvBlockLast, self).__init__()
        self.deconv = torch.nn.ConvTranspose2d(input_size, output_size,
                                               kernel_size, stride,
                                               padding)
        self.bn = torch.nn.BatchNorm2d(output_size)
...
class Generator(torch.nn.Module):
    def __init__(self, input_dim, num_filter, output_dim):
        ...
        self.deconv6 = DeconvBlock(num_filter * 4 * 2, num_filter * 2, 
                                   dropout = True)
        self.deconv7 = DeconvBlockLast(num_filter * 2 * 2, num_filter)
        self.deconv8 = DeconvBlockLast(num_filter * 2, output_dim,
                                       batch_norm=False)
    ...
    def normal_weight_init(self, mean=0.0, std=0.02):
        for m in self.children():
            if isinstance(m, ConvBlock):
                torch.nn.init.normal(m.conv.weight,
                                     mean, std)
            if isinstance(m, DeconvBlock):
                torch.nn.init.normal(m.deconv.weight,
                                     mean, std)
